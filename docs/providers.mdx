...
The `tokens` field is the sum of prompt and completion tokens; the `usage` field provides the detailed breakdown. `estimated_usd` is reserved for future cost calculation features and is currently always `null`.
